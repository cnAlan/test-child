...
@api_v3_app.route('/workflow/orgs/<org>/alert/dingtalk/watch/watchers/suggested')
def get_org_warning_watch_suggested_user(org: str):
    """
    返回该组织的推荐关注用户列表
    :param org:
    :return:
    ...
    if repo_count == 0:
        result = {
            'error': 'no repo for {owner}'.format(owner=org)
        }
...
@api_v3_app.route('/workflow/orgs/<org>/alert/dingtalk/watch/watchers')
def get_org_warning_dingtalk_watch_users(org: str):
    """
    返回关注某组织下的项目的用户
    :param org:
    :return:
...
@api_v3_app.route('/workflow/orgs/<org>/alert/dingtalk/watch/watcher/<user>', methods=['POST'])
def create_org_dingtalk_watcher(org, user):
    """
    user关注owner/repo项目

    POST /orgs/<owner>/alert/dingtalk/watch/watcher/<user>
    ...
    """
    # check owner
    query_user_result = Owner.query_owner_by_owner_name(db.session, org)
...
@api_v3_app.route('/workflow/orgs/<org>/alert/dingtalk/watch/watcher/<user>', methods=['DELETE'])
def delete_org_dingtalk_watcher(org, user):
    """
    取消user对owner所有项目的关注

    DELETE /orgs/<owner>/alert/dingtalk/watch/watcher/<user>
