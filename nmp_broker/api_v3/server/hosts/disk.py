...
@api_v3_app.route('/server/hosts/<host>/disk/space', methods=['POST'])
def receive_disk_space_message(host):
...
@api_v3_app.route('/server/hosts/<host>/disk/space', methods=['GET'])
def get_disk_space_message(host):
    start_time = datetime.datetime.utcnow()
