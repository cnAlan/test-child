...
export default class WatcherList extends React.Component{
  ...
  handleUnWatchClick(owner, user, event) {
    let users = [user];
    this.props.unwatch_click_handler(owner, users)
  }

  handleWatchClick(owner, user, event) {
    let users = [user];
    this.props.watch_click_handler(owner, users)
  }
  ...
  handleCheckedWatchClick(event){
    ...
    this.props.watch_click_handler(owner, this.state.checked_users);
  }

  handleCheckedUnWatchClick(event){
    const { owner } = this.props;
    this.props.unwatch_click_handler(owner, this.state.checked_users);
  }
              ...
              </label>
              {
                an_user.is_watching?
                  (<button className="btn btn-danger btn-sm active float-right"
                           onClick={this.handleUnWatchClick.bind(this, owner, an_user.owner_name)}>
                    取消
                  </button>) :
                  (<button className="btn btn-primary btn-sm float-right"
                           onClick={this.handleWatchClick.bind(this, owner, an_user.owner_name)}>
                    关注
                  </button>)
              }
              <span className="float-right">
                <span className="badge">{ an_user.watching_repo_count === undefined?'未知':an_user.watching_repo_count }</span>
                &nbsp;/&nbsp;
                <span className="badge">{ repo_count }</span>
                &nbsp;
                </span>
            </li>
          )}
